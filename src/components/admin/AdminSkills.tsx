...
function SortableSkillItem({ skill, onEdit, onDelete }: { skill: Skill, onEdit: () => void, onDelete: () => void }) {
  ...
  return (
    <motion.div ref={setNodeRef} style={style} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className={`glass-premium border border-white/[0.08]/50 angle-frame-sm p-3 flex items-center gap-4 group hover:border-white/[0.1] transition ${isDragging ? 'shadow-2xl ring-2 ring-purple-500 scale-[1.02]' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-gray-500 w-8">{skill.level}%</span>
        </div>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">{skills.length} habilidades</p>
        </div>
        <button onClick={openNew} className="flex items-center gap-2 px-4 py-2.5 angle-frame-sm font-semibold text-sm text-white bg-gradient-to-r from-purple-600 to-violet-500 hover:from-purple-500 hover:to-violet-400 transition shadow-lg shadow-purple-900/20 active:scale-95">
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="text-center py-16 bg-bg-secondary border border-white/[0.08] angle-frame">
          ...
          <p className="text-gray-600 text-sm">No hay skills. Agrega tus herramientas.</p>
        </div>
      ) : (
      ...
      <AnimatePresence>
        {showForm && editing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-bg-secondary border border-white/[0.08] angle-frame w-full max-w-md shadow-2xl">
              ...
              <div className="p-5 space-y-4">
                <div>
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1 block">Nombre</label>
                  <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
                    className="w-full bg-surface border border-white/[0.1] angle-frame-sm px-4 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500/50 transition" placeholder="Ej: After Effects" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1 block">Categoría</label>
                  <select value={editing.category} onChange={e => setEditing({ ...editing, category: e.target.value })}
                    className="w-full bg-surface border border-white/[0.1] angle-frame-sm px-4 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500/50 transition">
              ...
              <div className="flex justify-end gap-3 p-5 border-t border-white/[0.08]">
                ...
                <button onClick={handleSave} disabled={!editing.name.trim() || saving}
                  className="flex items-center gap-2 px-5 py-2.5 angle-frame-sm text-sm font-semibold text-white bg-gradient-to-r from-purple-600 to-violet-500 disabled:opacity-50 transition shadow-lg shadow-purple-900/20 active:scale-95">
