    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">{messages.length} mensajes · {unread} sin leer</p>
        </div>
        {unread > 0 && (
          <button
            onClick={markAllRead}
            className="flex items-center gap-2 px-4 py-2 angle-frame-sm text-sm text-gray-400 hover:text-white bg-bg-secondary border border-white/[0.08] hover:border-white/[0.1] transition"
          >
        ...
        <div className="lg:col-span-2 space-y-2 max-h-[65vh] overflow-y-auto pr-1">
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-gray-700 text-xs mt-1">Los mensajes del formulario de contacto aparecerán aquí</p>
            </div>
          ) : (
            filtered.map((msg, i) => (
              <motion.button
                ...
                onClick={() => openMsg(msg)}
                className={`w-full text-left p-4 angle-frame-sm border transition group ${
                  selected?.id === msg.id
                    ? 'bg-red-900/20 border-red-600/30'
                    : 'bg-bg-secondary border-white/[0.08] hover:border-white/[0.1]'
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${msg.read ? 'bg-surface-hover' : 'bg-red-500 animate-pulse'}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <p className={`text-sm truncate ${msg.read ? 'text-gray-400' : 'font-bold text-white'}`}>{msg.name}</p>
        ...
        <div className="lg:col-span-3">
          {selected ? (
            <motion.div
              ...
              className="bg-bg-secondary border border-white/[0.08] angle-frame p-6"
            >
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-neon-red to-neon-purple flex items-center justify-center">
              ...
              <div className="bg-surface/50 angle-frame-sm p-4 mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 angle-frame-sm text-sm text-gray-400 hover:text-white bg-surface hover:bg-surface-hover transition"
                >
                ...
                <a
                  ...
                  className="flex items-center gap-2 px-4 py-2 angle-frame-sm text-sm text-[#0B0E13] bg-neon-red hover:bg-[#E2FF75] transition"
                >
                  <Mail size={14} /> Responder
                </a>
                <button
                  onClick={() => deleteMsg(selected.id)}
                  className="flex items-center gap-2 px-4 py-2 angle-frame-sm text-sm text-red-400 hover:text-white hover:bg-red-900/30 transition ml-auto"
                >
            ...
            <div className="bg-bg-secondary border border-white/[0.08] angle-frame p-12 flex flex-col items-center justify-center text-center h-full min-h-[300px]">
              <Mail size={40} className="text-gray-700 mb-3" />
              <p className="text-gray-500 text-sm">Selecciona un mensaje para ver los detalles</p>
            </div>
          )}
        </div>
      </div>
