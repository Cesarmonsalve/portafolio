...
const SH = ({ id, title, icon: Icon, color, openSections, toggleSection }: {
  ...
  <button onClick={() => toggleSection(id)} className="w-full flex items-center justify-between p-4 rounded-t-2xl hover:bg-zinc-800/30 transition group">
...
const Slider = ({ label, value, onChange, min = 0, max = 100, suffix = '%' }: { label: string; value: number; onChange: (v: number) => void; min?: number; max?: number; suffix?: string }) => (
  <div>
    <div className="flex justify-between mb-1.5">
      ...
      <span className="text-xs text-zinc-500 font-mono">{value}{suffix}</span>
    </div>
    <input type="range" min={min} max={max} value={value} onChange={e => onChange(Number(e.target.value))}
      className="w-full h-1.5 bg-zinc-800 rounded-full appearance-none cursor-pointer accent-red-500" />
  </div>
);
...
const Toggle = ({ label, value, onChange, desc }: { label: string; value: boolean; onChange: (v: boolean) => void; desc?: string }) => (
  <div className="flex items-center justify-between py-2">
    ...
    <button onClick={() => onChange(!value)}
      className={`w-10 h-5 rounded-full transition-colors relative ${value ? 'bg-red-600' : 'bg-zinc-700'}`}>
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Fondos, videos, efectos, tipografía y más</p>
        </div>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-5 pb-5 space-y-6">
                {sectionKeys.map(sk => {
                  ...
                  return (
                    <div key={sk} className="bg-zinc-800/30 rounded-xl p-4 border border-zinc-700/30">
                        ...
                        {BG_TYPES.map(t => {
                          ...
                          return (
                            <button key={t.id} onClick={() => updateBg(sk, 'type', t.id)}
                              className={`flex flex-col items-center gap-1 py-2.5 rounded-xl text-xs font-medium transition ${active ? 'bg-red-600/20 text-red-400 border border-red-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30 hover:border-zinc-600'}`}>
                              <Icon size={16} />
                              {t.label}
                            </button>
                          );
                      ...
                      {bg.type === 'color' && (
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-zinc-500 font-mono">{bg.color || '#060606'}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {bg.type === 'gradient' && (
                        <div>
                          <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">Degradado CSS</label>
                          <input value={bg.gradient || ''} onChange={e => updateBg(sk, 'gradient', e.target.value)}
                            placeholder="linear-gradient(135deg, #0a0a0a, #1a0a1a)"
                            className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm font-mono focus:outline-none focus:border-red-500/50 transition" />
                          {bg.gradient && (
                            <div className="mt-2 h-8 rounded-lg border border-zinc-700" style={{ background: bg.gradient }} />
                          )}
                        </div>
                      )}
                      ...
                      {bg.type === 'image' && (
                        <div className="space-y-3">
                          <div>
                            <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">URL de Imagen</label>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-zinc-400">Overlay</span>
                          </div>
                          <Slider label="Opacidad de overlay" value={bg.overlayOpacity ?? 60} onChange={v => updateBg(sk, 'overlayOpacity', v)} />
                          {bg.imageUrl && (
                            <div className="mt-2 h-20 rounded-lg border border-zinc-700 overflow-hidden relative">
                      ...
                      {bg.type === 'video' && (
                        <div className="space-y-3">
                          <div>
                            <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">URL de Video (MP4/WebM)</label>
                          ...
                          <div className="flex items-center gap-3">
                            <input type="color" value={bg.overlayColor || '#000000'} onChange={e => updateBg(sk, 'overlayColor', e.target.value)}
                              className="w-8 h-8 rounded-lg border border-zinc-700 bg-zinc-800 cursor-pointer" />
                            <span className="text-xs text-zinc-400">Color overlay</span>
                          </div>
                          <Slider label="Opacidad de overlay" value={bg.overlayOpacity ?? 60} onChange={v => updateBg(sk, 'overlayOpacity', v)} />
                          <p className="text-[10px] text-zinc-600">💡 Usa videos cortos (5-15s), muted, en loop. Formatos: MP4 o WebM.</p>
                        </div>
                      )}
                    </div>
                  );
          ...
          {openSections.sections && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-5 pb-5 space-y-2">
                <p className="text-[10px] text-zinc-600 mb-3">Muestra/oculta secciones y cambia su orden. El número más bajo aparece primero.</p>
                {sectionKeys.map(sk => {
                  const sv = getSectionVisual(sk);
                  return (
                    <div key={sk} className="flex items-center gap-3 bg-zinc-800/30 rounded-xl p-3 border border-zinc-700/30">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        }} className="p-1 text-zinc-500 hover:text-white transition"><ArrowUp size={14} /></button>
                        <span className="text-xs text-zinc-500 font-mono w-6 text-center">{sv.position ?? 0}</span>
                        <button onClick={() => {
                          const pos = sv.position ?? 0;
                          if (pos < 4) updateSectionVisual(sk, 'position', pos + 1);
                        }} className="p-1 text-zinc-500 hover:text-white transition"><ArrowDown size={14} /></button>
                      </div>
                      <button onClick={() => updateSectionVisual(sk, 'visible', !sv.visible)}
                        className={`p-1.5 rounded-lg transition ${sv.visible ? 'text-green-400 bg-green-900/20' : 'text-zinc-600 bg-zinc-800'}`}>
          ...
          {openSections.typography && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-5 pb-5 space-y-4">
                {/* Display font */}
                <div>
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1.5 block">Fuente Display (Títulos)</label>
                  <select value={cfg.font_display || 'Syne'} onChange={e => setCfg(p => ({ ...p, font_display: e.target.value }))}
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-amber-500/50 transition">
                    {FONT_OPTIONS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                  <div className="mt-2 p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/30">
                ...
                {/* Body font */}
                <div>
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1.5 block">Fuente Body (Cuerpo)</label>
                ...
                {/* Heading size */}
                <div>
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1.5 block">Tamaño de Títulos</label>
                  <div className="grid grid-cols-4 gap-2">
                    {(['sm', 'md', 'lg', 'xl'] as const).map(sz => (
                      <button key={sz} onClick={() => setCfg(p => ({ ...p, heading_size: sz }))}
                        className={`py-2 rounded-xl text-sm font-bold uppercase transition ${cfg.heading_size === sz ? 'bg-red-600/20 text-red-400 border border-red-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30 hover:border-zinc-600'}`}>
                        {sz}
                      </button>
                    ))}
                  </div>
              ...
              <div className="px-5 pb-5 space-y-6 mt-4">
                 ...
                 <div className="bg-zinc-800/30 rounded-xl p-4 border border-zinc-700/30 space-y-3">
                   ...
                   <div className="grid grid-cols-2 gap-2 mb-2">
                     <button onClick={() => setCfg(p => ({ ...p, hero_name_type: 'text' }))} className={`py-2 rounded-xl text-xs font-medium transition ${cfg.hero_name_type !== 'image' ? 'bg-blue-600/20 text-blue-400 border border-blue-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30'}`}>Texto Glitch</button>
                     <button onClick={() => setCfg(p => ({ ...p, hero_name_type: 'image' }))} className={`py-2 rounded-xl text-xs font-medium transition ${cfg.hero_name_type === 'image' ? 'bg-blue-600/20 text-blue-400 border border-blue-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30'}`}>Imagen (PNG/Logo)</button>
                   </div>
                   
                   {cfg.hero_name_type === 'image' ? (
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">URL del Logotipo (PNG)</label>
                       <input value={cfg.hero_name_image || ''} onChange={e => setCfg(p => ({ ...p, hero_name_image: e.target.value }))} placeholder="https://... o /logo.png" className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition mb-3" />
                       {cfg.hero_name_image && <img src={cfg.hero_name_image} alt="Logo preview" className="h-12 object-contain bg-zinc-800/50 rounded-lg p-2 border border-zinc-700/30" />}
                     </div>
                   ) : (
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">Texto del Título</label>
                       <input value={cfg.hero_name || ''} onChange={e => setCfg(p => ({ ...p, hero_name: e.target.value }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition mb-3" />
                     </div>
                   )}
                   
                   <div className="grid grid-cols-2 gap-4">
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">Alineación</label>
                       <select value={cfg.hero_name_align || 'center'} onChange={e => setCfg(p => ({ ...p, hero_name_align: e.target.value as any }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition">
                 ...
                 <div className="bg-zinc-800/30 rounded-xl p-4 border border-zinc-700/30 space-y-3">
                   ...
                   <div className="grid grid-cols-2 gap-2 mb-2">
                     <button onClick={() => setCfg(p => ({ ...p, about_heading_type: 'text' }))} className={`py-2 rounded-xl text-xs font-medium transition ${cfg.about_heading_type !== 'image' ? 'bg-blue-600/20 text-blue-400 border border-blue-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30'}`}>Texto</button>
                     <button onClick={() => setCfg(p => ({ ...p, about_heading_type: 'image' }))} className={`py-2 rounded-xl text-xs font-medium transition ${cfg.about_heading_type === 'image' ? 'bg-blue-600/20 text-blue-400 border border-blue-600/30' : 'bg-zinc-800 text-zinc-500 border border-zinc-700/30'}`}>Imagen (PNG/Logo)</button>
                   </div>
                   
                   {cfg.about_heading_type === 'image' ? (
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">URL del Logotipo (PNG)</label>
                       <input value={cfg.about_heading_image || ''} onChange={e => setCfg(p => ({ ...p, about_heading_image: e.target.value }))} placeholder="https://... o /logo.png" className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition mb-3" />
                       {cfg.about_heading_image && <img src={cfg.about_heading_image} alt="Logo preview" className="h-12 object-contain bg-zinc-800/50 rounded-lg p-2 border border-zinc-700/30" />}
                     </div>
                   ) : (
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">Texto del Título</label>
                       <input value={cfg.about_heading || ''} onChange={e => setCfg(p => ({ ...p, about_heading: e.target.value }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition mb-3" />
                     </div>
                   )}
                   ...
                   <div className="grid grid-cols-2 gap-4">
                     <div>
                       <label className="text-xs text-zinc-400 font-semibold uppercase tracking-widest mb-1.5 block">Alineación</label>
                       <select value={cfg.about_heading_align || 'left'} onChange={e => setCfg(p => ({ ...p, about_heading_align: e.target.value as any }))} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50 transition">
                         <option value="left">Izquierda</option>
                         <option value="center">Centro</option>
                         <option value="right">Derecha</option>
                       </select>
              ...
              <div className="px-5 pb-5 space-y-3">
                ...
                {(cfg.marquee_items || []).map((item, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    }} className="flex-1 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-pink-500/50 transition" />
                    <button onClick={() => {
                      const items = [...(cfg.marquee_items || [])];
                      items.splice(i, 1);
                      setCfg(p => ({ ...p, marquee_items: items }));
                    }} className="p-1.5 rounded-lg text-zinc-500 hover:text-red-400 hover:bg-red-900/20 transition text-xs">✕</button>
                  </div>
                ))}
                <button onClick={() => setCfg(p => ({ ...p, marquee_items: [...(p.marquee_items || []), 'Nuevo Item'] }))}
                  className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-dashed border-zinc-700 text-zinc-500 hover:text-white hover:border-zinc-500 transition text-sm">
                  + Agregar Item
                </button>
              </div>
            </motion.div>
